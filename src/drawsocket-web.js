 ...
 * split code to be easier to read and bundle afterwards
 * include mediasoup wrapper in drawsocket
 *   make callback for mediasoup that replaces the addVideoAudio function
 */
...
function processJSON_file(_obj) {
  ...
  for (let node of _obj) {
    if (node.hasOwnProperty('fetch')) {
      ...
      console.log('fetching', node.fetch);
...
function setConnectionCallback(cb) {
...
function init(socket_)
{
  ...
  // force state request (called from Max to force update)
  // server responds with state object (not a feedback loop)
  socket.on('statereq', ()=> {
